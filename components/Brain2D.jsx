...
class Brain2D extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: true,
  ...
  componentDidUpdate() {
    if (
      ...
      console.log("loadedActionButtons");
      this.setState({
  ...
  afterDrawChart = (chart) => {
    var ctx = chart.chart && chart.chart.ctx;
    if (!ctx) return;
  ...
  render() {
    ...
    const options = {
      ...
      scales: {
        y: {
          ...
          title: {
            display: true,
            text: "Number of MASxSR 7.5  Thresholds Exceeded",
          },
          ticks: {
            min: 0,
          },
    ...
    return (
      ...
      >
        ...
        <div className="col-md-4" ref={(ref) => (this.chartContainer = ref)}>
          <Bar data={data} options={options} plugins={this.plugins} />
          <div className="btn btn-primary action-btn-container">{actionButtons}</div>
          <div>
